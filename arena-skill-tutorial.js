(()=>{
  ...
  function syncReset(){try{if(Number(game?.kills||0)<=0)localStorage.removeItem(key())}catch{}}
  function installStyle(){
    ...
    const s=document.createElement('style');s.id='arena-skill-tutorial-style';s.textContent=`
      ...
      .skill-tutorial-icon{font-size:48px;line-height:1;margin-bottom:12px}.skill-tutorial-window h2{margin:0 0 8px;font-family:Cinzel,serif;color:#e3c46e}.skill-tutorial-window p{margin:0 auto 16px;max-width:420px;color:#969ba3;font-size:.76rem;line-height:1.65}.skill-tutorial-rule{display:grid;grid-template-columns:1fr auto 1fr;gap:8px;align-items:center;margin:18px 0;color:#676d76;font-size:.58rem;letter-spacing:1px}.skill-tutorial-rule:before,.skill-tutorial-rule:after{content:'';height:1px;background:#35383d}.skill-tutorial-key{display:inline-flex;align-items:center;justify-content:center;min-width:100px;height:48px;padding:0 18px;border:1px solid #8c713c;background:linear-gradient(145deg,#302719,#17130e);color:#e3c46e;font-weight:900;font-size:.8rem;letter-spacing:1px;box-shadow:inset 0 0 20px rgba(215,183,94,.06)}.skill-tutorial-close{margin-top:18px;width:100%}`;document.head.appendChild(s);
  }
  function show(){
    syncReset();
    ...
    const close=()=>{m.remove();document.removeEventListener('keydown',esc)};const esc=e=>{if(e.key==='Escape')close()};
    m.querySelector('.skill-tutorial-backdrop').onclick=close;m.querySelector('#skillTutorialGo').onclick=()=>{close();setTimeout(()=>document.getElementById('skillTrainBtn')?.click(),80)};document.addEventListener('keydown',esc);
  }
  function patch(){
    if(typeof renderAll!=='function'||window.__arenaSkillTutorialPatch)return;
    window.__arenaSkillTutorialPatch=true;const old=renderAll;window.renderAll=function(...args){const r=old(...args);setTimeout(show,450);return r};
  }
  installStyle();patch();setTimeout(show,800);
  const watcher=setInterval(show,300);setTimeout(()=>clearInterval(watcher),30000);
})();
