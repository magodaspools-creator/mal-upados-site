// Arena Visual Test — real CC0 pixel-art assets + grounded combat animation
(()=>{
 ...
 const PLAYER_FRONT='https://raw.githubusercontent.com/pierpo/phaser3-simple-rpg/master/assets/spritesheets/hero/idle/hero-idle-front.png';
 const PLAYER_BACK='https://raw.githubusercontent.com/pierpo/phaser3-simple-rpg/master/assets/spritesheets/hero/idle/hero-idle-back.png';
 const NPC='https://raw.githubusercontent.com/pierpo/phaser3-simple-rpg/master/assets/npc.png';
 let forest=null,walk=null,attack=null,attackWeapon=null,front=null,back=null,npc=null,ready=false;
 let lastLog='',attackUntil=0,hitUntil=0,lastHp='';
 ...
 function setup(){css();const battle=document.querySelector('#arenaGameMode .arena-game-battle');if(!battle||document.getElementById(ID))return !!battle;const wrap=document.createElement('div');wrap.className='arena-scene-wrap';wrap.innerHTML=`<canvas id="${ID}" width="1280" height="560"></canvas><div class="arena-scene-hud"><span class="arena-scene-label">ARENA · FLORESTA</span><span class="arena-scene-label" id="arenaSceneWave">WAVE 1</span></div>`;battle.parentNode.insertBefore(wrap,battle);const note=document.createElement('div');note.className='arena-scene-note';note.textContent='Pixel-art CC0 · caminhada, ataque e impacto';wrap.after(note);return true}
 function loadAssets(){
  const assets=[FOREST,PLAYER_WALK,PLAYER_ATTACK,PLAYER_ATTACK_WEAPON,PLAYER_FRONT,PLAYER_BACK,NPC].map(img);
  [forest,walk,attack,attackWeapon,front,back,npc]=assets;let n=0;const done=()=>{if(++n>=assets.length){ready=true;render()}};assets.forEach(i=>{i.onload=done;i.onerror=done});
 }
 ...
 function playerSprite(c,x,y,attackState,frame){
  const fw=32,fh=32,scale=3.0,w=fw*scale,h=fh*scale;
  let sheet=walk,row=0,col=0;
  // The real ansimuz pack has separate front/back/side sheets. During combat the player faces the enemy (right).
  if(attackState){sheet=attackWeapon&&attackWeapon.complete&&attackWeapon.naturalWidth?attackWeapon:attack;col=frame%3}
  else {sheet=walk;col=frame%3}
  groundShadow(c,x,y+1,38);
  const t=attackState?Math.min(1,(Date.now()-(attackUntil-420))/420):0;
  const lunge=attackState?34*Math.sin(Math.PI*t):0;
  c.save();c.translate(x+lunge,y);c.imageSmoothingEnabled=false;c.drawImage(sheet,col*fw,row*fh,fw,fh,-w/2,-h,w,h);c.restore();
 }
 function npcSprite(c,x,y,frame,hit){
  const fw=16,fh=16,scale=3.0,w=fw*scale,h=fh*scale;
  groundShadow(c,x,y+1,38);
  const shake=hit?Math.sin(Date.now()/35)*5:0;
  c.save();c.translate(x+shake,y);c.imageSmoothingEnabled=false;
  // NPC sheet is only used for its actual 16x16 frames; no artificial continuous rotation.
 ...
 function render(){
  ...
  if(log!==lastLog&&log.includes('dano')){attackUntil=Date.now()+420;hitUntil=Date.now()+260}lastLog=log;
  const hpChanged=lastHp!==hpText;if(hpChanged&&lastHp)hitUntil=Date.now()+220;lastHp=hpText;
  const now=Date.now(),attacking=now<attackUntil,hit=now<hitUntil,frame=Math.floor(now/120)%3;
  const groundY=H*.82;
  playerSprite(c,W*.29,groundY,attacking,attacking?Math.floor((420-(attackUntil-now))/140):frame);
  npcSprite(c,W*.71,groundY,frame,hit);
  if(attacking){const p=Math.min(1,(now-(attackUntil-420))/420),ix=W*.29+(W*.42)*p; c.save();c.globalAlpha=Math.sin(Math.PI*p);c.fillStyle='#ffe9a3';c.beginPath();c.arc(ix,groundY-55,11+7*Math.sin(Math.PI*p),0,Math.PI*2);c.fill();c.fillStyle='#fff7d2';c.fillRect(ix-3,groundY-58,6,6);c.restore()}
