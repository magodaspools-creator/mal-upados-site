(()=>{
  ...
  if(typeof oldStart==='function')window.startBattle=function(zoneIndex,monsterIndex){
    ...
    if(b && Number(zoneIndex)===ZONES.length-1){
      b.isCursedRuins=true;
      b.corruption=0;
      b.corruptionSurge=false;
      b.corruptionRewarded=false;
      window.__arenaBattleRef=b;
      renderCorruption();
    }
    return out;
  };
  ...
  function watchVictory(){
    ...
    const obs=new MutationObserver(()=>{
      const b=getBattle();
      if(b&&b.isCursedRuins)return;
      if(!game)return;
      if(game.zone!==ZONES.length-1)return;
      if(document.body.dataset.cursedRewarded==='1')return;
      const text=area.textContent||'';
      if(/Vitória|Vitoria|derrotado/i.test(text)){
        document.body.dataset.cursedRewarded='1';
        const corruption=Number(window.__lastCursedCorruption||0);
        const bonus=Math.floor((Number(game.gold)||0)*0);
        if(corruption>0){
          game.gold=(Number(game.gold)||0)+Math.floor(1000*(corruption/100));
  ...
  function addMapCard(){
    style();
    const mapView=document.getElementById('arenaViewMap');if(!mapView)return;
    if(document.getElementById('cursedRuinsMapCard'))return;
    const card=document.createElement('section');card.id='cursedRuinsMapCard';card.className='cursed-map-card';
    card.innerHTML=`<h3>☠ Ruínas Amaldiçoadas</h3><p>Uma região fora da lógica normal da Arena. Aqui você não entra para repetir o mesmo combate: você decide até onde consegue levar a Corrupção antes que ela volte contra você.</p><div class="rule"><div><b>Corrupção</b><span>+12% por ataque. Mais Corrupção = mais dano, risco e Gold.</span></div><div><b>Surto</b><span>Em 100%, o inimigo recupera 18% da vida e a luta entra em estado crítico.</span></div><div><b>Recompensa</b><span>O Gold extra cresce com a Corrupção acumulada na luta.</span></div></div>`;
    mapView.appendChild(card);
  }

  style();
  const timer=setInterval(()=>{addMapCard();renderCorruption()},300);
