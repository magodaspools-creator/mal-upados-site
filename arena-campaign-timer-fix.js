// Timer robusto da campanha: não depende de window.game/battle.
(()=>{
  ...
  const fmt=ms=>{let s=Math.floor(ms/1000),h=Math.floor(s/3600);s%=3600;let m=Math.floor(s/60);s%=60;return `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`};
  const ensure=()=>{if(document.getElementById('campaignTimer'))return;const top=document.querySelector('.game-topbar');if(!top)return;const e=document.createElement('div');e.id='campaignTimer';e.innerHTML='<div class="campaign-timer-card"><div><div class="eyebrow">Campanha normal</div><strong id="campaignTimerValue">00:00:00</strong></div><span id="campaignTimerStatus">Ainda não iniciada</span></div>';top.insertAdjacentElement('afterend',e)};
  const render=()=>{ensure();const v=document.getElementById('campaignTimerValue'),s=document.getElementById('campaignTimerStatus'),box=document.querySelector('.campaign-timer-card');if(!v||!s)return;const x=read();if(!x){v.textContent='00:00:00';s.textContent='Ainda não iniciada';return}v.textContent=fmt((x.completedAt||Date.now())-x.startedAt);s.textContent=x.completedAt?'Campanha concluída':'Contando · sem pausa';box?.classList.toggle('done',!!x.completedAt)};
  const start=()=>{const x=read();if(!x?.startedAt)write({version:2,character:char(),startedAt:Date.now(),completedAt:null});render()};
