...
/* Explicit original Tibia levels for official items; Arena requirement is derived from these. */
(()=>{
 const tibiaLevels={
  "199464":35,
  "199465":35,
 ...
 const priceFromTibiaLevel=level=>Math.round(1200+(120*level)+(0.45*level*level));
 const arenaLevelFromTibia=level=>level<=50?5:level<=80?10:level<=100?15:level<=120?20:level<=150?25:level<=180?30:level<=200?35:level<=230?40:level<=250?45:level<=270?50:level<=300?60:70;
 for(const [id,level] of Object.entries(tibiaLevels)){
  const item=SHOP_ITEMS.find(x=>String(x.id||'')===id||String(x.id||'').endsWith('-'+id)||String(x.sprite||'').split('/').pop()===id+'.png'||x.name===id);
  if(!item)continue;
  item.tibiaLevel=level;
  if(id!=='214974'){
   item.minLevel=arenaLevelFromTibia(level);
   item.price=priceFromTibiaLevel(level);
  }
 }
})();