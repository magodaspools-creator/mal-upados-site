(()=>{
  ...
  const CSS=`
    ...
    .arena-view[data-view="map"] .arena-map-view-head,.arena-view[data-view="forge"] .arena-forge-view-head{margin-bottom:16px}
    .arena-map-view-head,.arena-forge-view-head{display:flex;justify-content:space-between;align-items:end;gap:18px;padding:0 2px}
    ...
    .arena-view .p4-command{margin-top:0}
    .arena-view .activities{padding-top:0}
    .arena-view .shop-section+.shop-section{margin-top:16px}
    .arena-view[data-view="daily"] .p3-codex{margin-top:16px}
    .arena-view[data-view="progress"] .progress-section{margin-top:0}
    .arena-view[data-view="ranking"] .arena-ranking{margin-top:0}
  ...
  function setup(){
    ...
    // O shell deixa de ser uma página longa: vira exclusivamente a tela de combate.
    const combat=mk('arenaViewCombat','combat');
    move(shell,combat);

    // O mapa superior aprovado ganha sua própria tela.
    ...
    const mapControls=mapSource?.querySelector('.map-controls');
    const map=mapSource?.querySelector('#map');
    // Mantém os controles existentes, mas dentro da nova tela.
    if(mapSectionHead){
      const title=mapSectionHead.querySelector('h2');if(title)title.textContent='Escolha seu destino';
      mapBody.appendChild(mapSectionHead);
    }
    if(map)mapBody.appendChild(map);
    mapView.append(mapHead,mapBody);

    // A Fase 4 inteira vira a tela de Forja/estado da jornada.
    const forgeView=mk('arenaViewForge','forge');
    const command=$('#arenaPhase4Command');
    if(command)forgeView.appendChild(command);
    else forgeView.innerHTML='<div class="arena-view-empty">Forja carregando...</div>';

    const dailyView=mk('arenaViewDaily','daily');
    const activities=$('.activities');
    if(activities)dailyView.appendChild(activities);
    const progressView=mk('arenaViewProgress','progress');
    const progress=$('.progress-section');
    if(progress)progressView.appendChild(progress);
    const rankingView=mk('arenaViewRanking','ranking');
    const ranking=$('.arena-ranking');
    if(ranking)rankingView.appendChild(ranking);
    ...
    // Insere no lugar dos blocos antigos.
    main.appendChild(views);

    // O Codex da Fase 3 nasce depois. Ele entra na tela Diário automaticamente.
    const codex=$('#arenaPhase3Codex');if(codex)dailyView.appendChild(codex);

    // O hero permanece fixo; tudo abaixo dele agora é uma única view por vez.
    nav.querySelectorAll('button').forEach(btn=>{
      const old=btn.dataset.target;
      const map={arenaCombatSection:'arenaViewCombat',arenaMap:'arenaViewMap',arenaForgeSection:'arenaViewForge',arenaActivitiesSection:'arenaViewDaily',arenaShopSection:'arenaViewShop',arenaProgressSection:'arenaViewProgress',arenaRankingSection:'arenaViewRanking'};
      btn.dataset.viewTarget=map[old]||old;
  ...
  function watch(){
    if(!setup())return;
    // Codex/forja podem ter sido criados depois da primeira montagem.
    const daily=$('#arenaViewDaily');const forge=$('#arenaViewForge');
    const codex=$('#arenaPhase3Codex');if(daily&&codex&&codex.parentElement!==daily)daily.appendChild(codex);
    const command=$('#arenaPhase4Command');if(forge&&command&&command.parentElement!==forge)forge.appendChild(command);
  }
