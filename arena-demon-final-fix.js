...
(()=>{
  ...
  const popupKey=()=>{
    try{
      const name=game?.character||document.getElementById('characterPickerName')?.textContent||'default';
      return `malupados_demon_popup_seen_${String(name).trim()||'default'}`;
    }catch{return 'malupados_demon_popup_seen_default'}
  };

  function campaignPopup(){
    // O marco Demon -> Deathbringer aparece somente na primeira vitória do Demon
    // para cada personagem. Depois disso, nunca interrompe novamente a campanha.
    try{
      const key=popupKey();
