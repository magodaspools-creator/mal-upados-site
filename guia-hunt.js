...
function huntCurrentRows(mode,voc,level,search){
  ...
  return source.filter(h=>{
    const levelOk=mode==='solo'
      ? h.min>=Math.max(8,level-50) && h.min<=level
      : h.min>=level;
...
function guiaRender(){
  ...
  const filtered=huntCurrentRows(mode,voc,level,search),score=guiaScore(filtered,goal),rows=mode!=='solo'
    ? [...filtered].sort((a,b)=>a.min-b.min||String(a.n).localeCompare(String(b.n)))
    : [...filtered].sort((a,b)=>score.get(b)-score.get(a)||((b.xp??-Infinity)-(a.xp??-Infinity))||b.min-a.min);
