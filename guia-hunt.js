...
function guiaRender(){
  ...
  stats.innerHTML=`<div class="stat"><div class="num">${rows.length}</div><div class="label">HUNTS ENCONTRADAS</div></div><div class="stat"><div class="num">${groupMode?'—':(maxXp?huntFmt(maxXp):'—')}</div><div class="label">MAIOR RAW/H</div></div><div class="stat"><div class="num">${groupMode?'—':(maxLoot?huntFmt(maxLoot):'—')}</div><div class="label">MAIOR LOOT/H</div></div><div class="stat"><div class="num">${level}</div><div class="label">SEU LEVEL</div></div>`;
  ...
  }else{const topXp=[...rows].filter(h=>h.xp!=null).sort((a,b)=>b.xp-a.xp).slice(0,3),topLoot=[...rows].filter(h=>h.loot!=null).sort((a,b)=>b.loot-a.loot).slice(0,3),topBalance=rows.slice(0,3),mini=(h,label)=>`<div class="hunt-mini"><span class="tag">${label}</span><strong>${esc(h.n)}</strong><span>Level ${h.min}+ · Raw ${huntFmt(h.xp)}/h · Loot ${huntFmt(h.loot)}/h</span>${huntVideoButton(h.n)}</div>`;highlights.innerHTML=`<div class="hunt-section-head"><div><div class="eyebrow">Destaques</div><h3>Três jeitos de olhar para a mesma base</h3></div><div class="small">Fonte: <a href="${TIBIAPAL_URL}" target="_blank" rel="noopener">TibiaPal Old</a></div></div><div class="hunt-highlight-grid"><div><h3>Mais Raw</h3>${topXp.map(h=>mini(h,'RAW')).join('')}</div><div><h3>Mais Loot</h3>${topLoot.map(h=>mini(h,'LOOT')).join('')}</div><div><h3>Equilíbrio</h3>${topBalance.map(h=>mini(h,'BALANÇO')).join('')}</div></div>`;}
  const title=mode==='solo'?`${voc} · Level ${level}`:mode==='duo'?'Duo':'Teamhunt',goalLabel=groupMode?'ordem por level mínimo':({xp:'Raw/h',profit:'Loot/h',balance:'equilíbrio Raw + loot'}[goal]);
  result.innerHTML=`<div class="hunt-result-head"><div><div class="eyebrow">${esc(title)}</div><h3>${rows.length} hunt${rows.length===1?'':'s'} encontrada${rows.length===1?'':'s'}</h3></div><div class="small">${goalLabel}</div></div><div class="hunt-list">${rows.map((h,i)=>`<article class="hunt-card"><div class="hunt-card-main"><div class="hunt-position">${i+1}</div><div><div class="hunt-title">${esc(h.n)}</div><div class="hunt-meta">Level ${h.min}+${h.style?' · '+esc(h.style):''}</div></div></div><div class="hunt-metrics"><div><span>Raw/h</span><strong>${groupMode?'—':huntFmt(h.xp)}</strong></div><div><span>Loot/h</span><strong>${groupMode?'—':huntFmt(h.loot)}</strong></div><div><span>${groupMode?'Fonte':'Margem'}</span><strong>${groupMode?'TibiaPal Old':(h.loot==null?'—':(h.loot<0?'Prejuízo':'Positivo'))}</strong></div></div><div class="hunt-card-foot"><span>${groupMode?'Tabela de grupo do TibiaPal':'Referência histórica do TibiaPal'}</span><span>${huntVideoButton(h.n)}</span>${i===0?'<span class="badge">PRIMEIRA OPÇÃO</span>':''}</div></article>`).join('')}</div>`;
}
