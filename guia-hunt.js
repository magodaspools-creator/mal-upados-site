...
function huntVideoButton(name){
  const url=huntVideos[name];
  return url?`<a class="hunt-video" href="${url}" target="_blank" rel="noopener">▶ Ver vídeo</a>`:'';
}
function guiaRender(){
  ...
  const filtered=huntCurrentRows(mode,voc,level,search),score=guiaScore(filtered,goal);
  ...
  if(groupMode){
    ...
    const topXp=[...rows].filter(h=>h.xp!=null).sort((a,b)=>b.xp-a.xp).slice(0,3),topLoot=[...rows].filter(h=>h.loot!=null).sort((a,b)=>b.loot-a.loot).slice(0,3),topBalance=rows.slice(0,3),mini=(h,label)=>`<div class="hunt-mini"><span class="tag">${label}</span><strong>${esc(h.n)}</strong><span>Level ${h.min}+ · XP ${huntFmt(h.xp)}/h · Loot ${huntFmt(h.loot)}/h</span>${huntVideoButton(h.n)}</div>`;
