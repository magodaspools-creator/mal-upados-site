(function(){
  ...
  const VERSION='2026.09.11-stage5';
  ...
  function imageDataFingerprint(imageData){
    ...
    for(let y=0;y<h;y++)for(let x=0;x<w;x++){
      ...
      if(a===0)bytes.push(0,0,0,0);else bytes.push(d[i],d[i+1],d[i+2],255);
    }
  ...
  async function loadEntryData(entry){
    ...
    const source=entry.image||entry.src;
    if(!source)return null;
    const img=await imageFromUrl(source);
    const data=normalizeSpriteSource(img);
  ...
  async function buildSpriteIndex(entries){
    ...
    for(let i=0;i<(Array.isArray(entries)?entries.length:0);i++){
      ...
      list.push({
        ...
        edge:Number(entry.edge||0),
        npcPrice:Number(entry.npcPrice||0)||0,
        marketPrice:Number(entry.marketPrice||0)||0
      });
    }
    const buckets=new Map();
    list.forEach(s=>{if(s.fingerprint){if(!buckets.has(s.fingerprint))buckets.set(s.fingerprint,[]);buckets.get(s.fingerprint).push(s);}});
  ...
  function coarseDistance(slot,s){
    return Math.abs(slot.mean-s.mean)*0.45+Math.abs(Math.sqrt(slot.variance)-Math.sqrt(Math.max(0,s.variance)))*0.35+Math.abs(slot.edge-s.edge)/1000*0.20;
  }
  ...
  async function matchSlot(slot,options={}){
    ...
    const ctx=slot.crop.getContext('2d',{willReadFrequently:true});
    const data=ctx.getImageData(0,0,TILE,TILE);
    const fp=imageDataFingerprint(data);
    ...
    for(const s of coarse){
      const spriteData=await loadEntryData(s);
      if(spriteData)ranked.push({s,distance:imageDistance(data,spriteData)});
    }
    ranked.sort((a,b)=>a.distance-b.distance);
    const candidates=ranked.slice(0,limit).filter(x=>x.distance<=threshold).map(x=>({id:x.s.id,name:x.s.name,distance:Number(x.distance.toFixed(3)),npcPrice:x.s.npcPrice,marketPrice:x.s.marketPrice}));
  ...
  async function matchAll(options={}){
    const out=[];
    for(let printIndex=0;printIndex<latestReports.length;printIndex++){
      const slots=latestReports[printIndex]?.stashExtraction?.slots||[];
      for(const slot of slots){out.push({...await matchSlot(slot,options),printIndex,row:slot.row,col:slot.col});}
    }
    return out;
  }
  ...
  function updatePanel(reports){
    ...
    box.innerHTML=`<strong>Etapa 5 · reconhecimento determinístico</strong><br><span class="stash-note">${s.totalSlots} slots extraídos em ${s.prints} print${s.prints===1?'':'s'} · ${db} · fingerprint exato + comparação pixel a pixel sob demanda · sem chute de nome.</span>`;
  }
