...
def extract_items(payload):
    ...
    for key in ("items","data","results","item"):
...
def load_catalog():
    ...
    for page in range(1,MAX_PAGES+1):
        url=f"{ITEMS_API}?page={page}&limit={PAGE_SIZE}"
        r=requests.get(url,timeout=30,headers={"User-Agent":"MalUpados-Stash-Builder/2.0","Accept":"application/json"})
        r.raise_for_status()
        ...
        for item in batch:
            ...
            if name and 1 <= item_id <= 100000: found[item_id]=str(name)
        print(f"Página {page}: {len(batch)} itens · catálogo acumulado: {len(found)}")
        if len(batch) < PAGE_SIZE: break
    if not found: raise RuntimeError("TibiaData não retornou IDs de itens.")
    return found


def fetch_one(item):
    item_id,name=item; url=f"{SPRITE_BASE}{item_id}.png"
    try:
        r=requests.get(url,timeout=20,headers={"User-Agent":"MalUpados-Stash-Builder/2.0"})
        ...
        return {"id":item_id,"name":name,"src":url,"fingerprint":fingerprint(img),"mean":mean,"variance":variance,"edge":edge,"marketable":True}
    except Exception:
        return None


def main():
    print("Baixando catálogo atual de itens via TibiaData...")
    items=load_catalog(); print(f"IDs encontrados: {len(items)}")
    out=[]
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        ...
        for i,future in enumerate(as_completed(futures),1):
            ...
            if i % 250 == 0: print(f"Sprites processados: {i}/{len(futures)} · válidos: {len(out)}")
    out.sort(key=lambda x:x["id"])
    OUT.parent.mkdir(parents=True,exist_ok=True)
    payload={"version":"2026.09.11-remote-15.10+","source":"TibiaData item catalog + item-images.ots.me","baseUrl":SPRITE_BASE,"generatedAt":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),"items":out}
