...
(()=>{
 ...
 const HERO_FRAMES=7;
 // The Six Kingdoms asset is stored as directional PNGs, not idle_south/00.png frames.
 const ENEMY_IDLE=ROOT+'six-kingdoms-free-v1.0/six-kingdoms-free-v1.0/01_coral_knight_SUNKEN/128/south.png';
 ...
 function style(){
  ...
  const s=document.createElement('style');s.id='arena-game-sprites-style';s.textContent=`
   ...
   #arenaGameMode .arena-fighter-icon{height:${HERO_SIZE}px;width:${HERO_SIZE}px;margin:auto;display:flex;align-items:center;justify-content:center;font-size:0;line-height:0;filter:drop-shadow(0 7px 7px rgba(0,0,0,.68));position:relative;overflow:visible;background-repeat:no-repeat;background-position:center;background-size:${HERO_SIZE}px ${HERO_SIZE}px}
 ...
 function draw(){const p=playerEl(),e=enemyEl();if(!p||!e)return;const v=playerVocation();paint(p,heroIdle(v,animFrame%HERO_FRAMES),HERO_SIZE);paint(e,ENEMY_IDLE,ENEMY_SIZE)}
