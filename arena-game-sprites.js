...
(()=>{
 ...
 const MAP_BG=ROOT+'Foozle Lucifer Dungeon Tileset/Foozle_2DT0003_Lucifer_Dungeon_Tileset_Pixel_Art/DungeonTileset Mockup.png';
 ...
 function style(){
  ...
  s.textContent=`
   /* O mapa é o palco REAL do Waves. Os cards viram HUD sobre o cenário. */
   ...
   #arenaGameMode .arena-game-battle:before{
    ...
    position:absolute;
    inset:0;
    background:linear-gradient(180deg,rgba(4,6,9,.20),rgba(4,6,9,.38));
    pointer-events:none;
    z-index:0;
   }
   ...
   #arenaGameMode .arena-fighter-icon{
    ...
    font-size:0;
    line-height:0;
    filter:drop-shadow(0 10px 9px rgba(0,0,0,.68));
    position:relative;
    overflow:visible;
   ...
   @media(max-width:650px){
    #arenaGameMode .arena-game-battle{min-height:330px}
    #arenaGameMode .arena-fighter{width:145px}
    #arenaGameMode .arena-fighter:first-child{left:1%}
    #arenaGameMode .arena-fighter.enemy{right:1%}
    #arenaGameMode .arena-fighter-icon{width:110px;height:110px;background-size:110px 110px}
    #arenaGameMode .arena-fighter.enemy .arena-fighter-icon{background-size:100px 100px}
   }
  `;
 ...
 function prepareAssets(){
  ...
  preload(ENEMY_IDLE);preload(MAP_BG);
 }
 function draw(){
  const p=playerEl(),e=enemyEl();if(!p||!e)return;
  const v=playerVocation();
  paint(p,heroIdle(v,animFrame%HERO_FRAMES),150);
  paint(e,ENEMY_IDLE,132);
 }
 function startAttack(){
  if(attacking)return;attacking=true;
  const p=playerEl();
  if(p){p.classList.remove('waves-attack');void p.offsetWidth;p.classList.add('waves-attack');setTimeout(()=>p.classList.remove('waves-attack'),300)}
 ...
 function observeDamage(){
  const t=document.getElementById('arenaEnemyHpText');if(!t)return;
  const m=String(t.textContent||'').match(/([\d.,]+)\s*\/\s*([\d.,]+)/);if(!m)return;
  const hp=Number(m[1].replace(/\./g,'').replace(',','.'));
  if(lastEnemyHp!==null&&hp<lastEnemyHp)enemyDamageFx();lastEnemyHp=hp;
 }
