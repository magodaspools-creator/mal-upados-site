...
(()=>{
  ...
  function hasAmulet(element){
    try{
      const amulet=window.arenaElementalState?.currentAmulet?.();
      return !!amulet&&amulet.element===element;
    }catch{return false}
  }
  ...
  window.showZone=function(index){
    ...
    const btn=area.querySelector('.boss-fight-btn');
  ...
  function startBoss(zoneIndex){
    ...
    const left=cooldownLeft(zoneIndex);if(left>0){toast(`Boss em cooldown: ${fmtTime(left)}.`);return}
    if(!hasAmulet(b.element)){toast(`Você precisa do amuleto de ${b.element} para enfrentar este Boss.`);return}
    const tuple=bossTuple(zoneIndex);
  ...
  function bossSprite(b){
    const colors={earth:'#7db77b',energy:'#8fa9e8',fire:'#e27b4f',ice:'#86cde5',death:'#a18ac7'};const c=colors[b.element]||'#aeb4bd';
    return `<svg class="arena-boss-sprite" viewBox="0 0 64 64" aria-hidden="true" shape-rendering="crispEdges"><path fill="#111318" d="M14 18h36v8h6v24H8V26h6z"/><path fill="${c}" d="M18 14h28v8h8v24H10V22h8z"/><path fill="#17191d" d="M18 30h8v8h-8zM38 30h8v8h-8z"/><path fill="#e8ebef" d="M20 31h4v4h-4zM40 31h4v4h-4z"/><path fill="#17191d" d="M24 44h16v6H24z"/><path fill="${c}" d="M8 18h8v10H8zM48 18h8v10h-8z"/></svg>`;
  }

  function renderBossBattle(){
    if(!battle?.isBoss){originalRenderBattle();return}
    const z=ZONES[battle.zoneIndex],b=bossFor(battle.zoneIndex);let pct=Math.max(0,battle.hp/battle.maxHp*100),ppct=Math.max(0,battle.playerHp/battle.playerMax*100);
    document.getElementById('battleArea').innerHTML=`<div class="battle-card boss-battle-card"><div class="battle-head"><div><div class="battle-zone">${esc(z.name)} · BOSS</div><h3>${esc(battle.name)}</h3></div><button class="btn" id="fleeBtn">Fugir</button></div><div class="boss-battle-warning">${b.icon} Proteção ${esc(b.element)} ativa · Boss +${battle.bossKills*5}% de força</div><div class="battle-enemies"><div class="fighter"><div class="fighter-icon">⚔️</div><h3>${esc(game.character)}</h3><div class="hp-track"><i style="width:${ppct}%"></i></div><div class="fighter-meta">HP ${fmt(battle.playerHp)} / ${fmt(battle.playerMax)}</div></div><div class="vs">VS</div><div class="fighter boss-fighter"><div class="fighter-icon boss-fighter-icon">${bossSprite(b)}</div><h3>${esc(battle.name)}</h3><div class="hp-track"><i style="width:${pct}%"></i></div><div class="fighter-meta">HP ${fmt(battle.hp)} / ${fmt(battle.maxHp)}</div></div></div><div class="battle-log">${battle.log.join('<br>')||'O Boss elemental encara você. A batalha começou.'}</div><div class="battle-actions"><button class="btn active big" id="attackBtn">ATACAR</button></div></div>`;
    document.getElementById('attackBtn').onclick=attack;
    document.getElementById('fleeBtn').onclick=()=>{battle=null;activeBoss=null;showZone(game.zone)};
  }
  ...
  window.winBattle=function(){
    ...
    activeBoss={zoneIndex:zone,name:b.name,tokenDrop};
    ...
    if(result){
      ...
      result.innerHTML=`<div class="battle-empty result"><div class="battle-icon">🏆</div><div class="battle-zone">${esc(ZONES[zone].name)} · BOSS DERROTADO</div><h3>${esc(b.name)} derrotado!</h3><p><strong class="loot">Boss Token</strong> · ${loot}</p><p class="small">Próxima tentativa em ${fmtTime(b.cooldown*1000)} · dificuldade futura +${Math.min(100,(oldKills+1)*5)}%</p><button class="btn active" id="againBtn">Voltar para a área</button></div>`;
  ...
  function installStyle(){
    if(document.getElementById('arena-boss-style'))return;
    const style=document.createElement('style');style.id='arena-boss-style';style.textContent=`
